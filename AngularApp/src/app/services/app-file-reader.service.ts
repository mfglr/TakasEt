...
export class AppFileReaderService {
  ...
  public getPosts(source : Uint8Array) : {post : PostResponse,extention : string,file : Uint8Array}[]{
    let sizeOfInt = this.byteArrayToNumber(source.subarray(0,1),1);
    let isBigEndian = this.byteArrayToNumber(source.subarray(1,2),1);
    let offset = 2;
    let ret : {post : PostResponse,extention : string,file : Uint8Array}[] = [];
    while(offset < source.length){
      
      let postLegth = this.readInt(source,offset,sizeOfInt,isBigEndian)
      offset += sizeOfInt;
      let post : PostResponse = JSON.parse(
        new TextDecoder("utf-8").decode( source.subarray(offset, offset += postLegth))
      )
      ...
      ret.push({post : post,extention : extention,file : file});
    }
    return ret;
  }
